...
while winner is None:
    for _ in range(3):
        for i in range(len(player_scores)):
            ...
            print(f"{biggest1 + biggest2} points were added to player {i+1}'s score")
            if biggest1 == biggest2:
                if biggest1 == 6:
                    player_scores[i] += 6
                    print(f"A bonus 6 points were added to player {i+1}'s score")
                else:
                    player_scores[i] += 5
                    print(f"A bonus 5 points were added to player {i+1}'s score")
            print()

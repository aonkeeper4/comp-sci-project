...
def fix_script(path):
    ...
    with open(path, 'rb') as script:
        ...
        if not firstline.startswith(b'#!python'):
            return False
        # Replit mod: when installing a binary, rely on the fact that we have
        # access to a modern 8.30+ version of  `env(1)` that can invoke an
        # executable using a relative path. This makes any binary generated by
        # this to be position-independent, so that it can be moved (or even the
        # repl forked!).
        exename = firstline[2:]
        firstline = b'#!/usr/bin/env -S ./' + exename + os.linesep.encode(
            "ascii")
        rest = script.read()
...
def _normalized_outrows(outrows):
    ...
    return sorted((ensure_str(record_path, encoding='utf-8'), hash_, str(size))
...
class ContentAddressablePool:
    ...
    def path_for_digest(self, digest):
        # type: (str) -> str
        return os.path.join(self.cache_dir, 'pool', digest[:2], digest[2:4],
                            digest[4:6], digest[6:])
    ...
    if pool is not None:
        for row in record_rows:
            ...
            if digest_name != 'sha256':
                continue
            digests[parsed_record_path] = urlsafe_b64decode(
                f'{b64hash}=').hex()
